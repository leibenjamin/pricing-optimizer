...
import type { CallbackDataParams, LabelFormatterCallback } from "echarts/types/dist/shared"

echarts.use([BarChart, GridComponent, TooltipComponent, TitleComponent, DatasetComponent, CanvasRenderer])

export type WaterStep = { label: string; delta: number }

// Compose a strict option type for this chart
type ECOption = ComposeOption<
  BarSeriesOption | GridComponentOption | TooltipComponentOption | TitleComponentOption | DatasetComponentOption
>
...
export function Waterfall({
  ...
  useEffect(() => {
    ...
    const labelFmt: LabelFormatterCallback<CallbackDataParams> = (p) => {
      ...
      else if (typeof raw === "object") v = Number((raw as { value?: unknown }).value ?? 0)
      else v = Number(raw)
      return `$${v.toFixed(2)}`
    }
      ...
      series: [
        ...
        },
        {
          ...
          label: {
            show: true,
            position: "top",
            // param is structurally typed; we only read 'value' (number | number[])
            formatter: labelFmt,
          },
        },
      ],
